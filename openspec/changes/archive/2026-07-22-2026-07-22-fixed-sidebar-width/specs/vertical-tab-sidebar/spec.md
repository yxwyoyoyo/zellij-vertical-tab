...
The development layout SHALL place a fixed-width sidebar in every tab, replace the built-in horizontal tab bar, retain the built-in status bar, and expose a native Zellij tiled-pane boundary that can resize the sidebar with the mouse.

#### Scenario: Session starts from the development layout
- **WHEN** Zellij loads `zellij.kdl`
- **THEN** every tab receives a borderless vertical sidebar with a fixed width of 32 columns
...
- **THEN** Zellij resizes the fixed-width sidebar and sibling content pane continuously with the pointer
...
- **THEN** the new tab receives the layout's configured fixed width of 32 columns
