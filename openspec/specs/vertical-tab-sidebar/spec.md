...
Every rendered tab row SHALL fit the sidebar's current content width and SHALL fill that width so selected styling spans the complete row.

#### Scenario: Tab name is short
- **WHEN** the formatted position and tab name use fewer cells than the available width
- **THEN** the plugin pads the row to the available width

#### Scenario: Tab name is long
- **WHEN** the formatted position and tab name exceed the available width
- **THEN** the plugin truncates the row to the available width
