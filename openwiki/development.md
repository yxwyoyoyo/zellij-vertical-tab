...
Reloading replaces the plugin instance, but the replacement restores pane-keyed lifecycle records and exact focus acknowledgements from `/cache/agent-status-<zellij-pid>/`; the first updated instance can migrate compatible legacy flat snapshots into that server shard. It then requests one host-journal snapshot through an installed Codex or Claude bridge helper so events emitted while no plugin runtime was available are reconciled by timestamp. If recovery is unavailable and the current pane, agent session ID, and state are known, republish that exact state through the normal `vertical-tab-agent-status` protocol:

```sh
mise run status -- terminal_0 <agent-session-id> done Hub
```

`scripts/publish-agent-status` accepts only `terminal_<number>` pane IDs and `idle`, `working`, `waiting`, `done`, or `clear`; it emits a version-1 payload with a fresh timestamp and uses the same explicit/environment/current-session targeting order. Starting another prompt also republishes state through the normal installed agent hooks. Durable host journaling begins at the next lifecycle event after the updated bridge is installed; it cannot reconstruct an older event that was never recorded. A stale WASM can make source-level debugging misleading (`tasks/lessons.md`), and startup-template or permission behavior still requires a fresh session rather than reload.
...
## Claude Code bridge installation

Install the Claude bridge and the shared durable store at user scope:
...
The [status synchronization and recovery architecture](architecture.md#sidebar-instance-convergence) must stay linear as the tab template adds sidebar instances:

- Zellij's untargeted lifecycle pipe supplies exactly one delivery per listening sidebar; recipients apply it locally and do not relay it.
- The lowest live plugin ID leads focus synchronization. Nonleaders report once to it, and it performs the only peer fanout. Counting nonleader reports plus leader focus and acknowledgement fanouts, the conservative bound is `3(N - 1)` peer messages for `N` sidebars.
- Late joiners recover from bounded peer snapshots rather than replaying lifecycle history.
- Repeated `SessionStart` events for one Codex PID may refresh watcher metadata but must leave at most one resident watcher; macOS should use event-driven `kqueue`, with two-second polling only as a fallback.
- Normal plugin restore scans only `/cache/agent-status-<zellij-pid>/`; legacy flat files are a one-time migration source. Session start removes only host-journal directories whose numeric PID is demonstrably dead.

When changing these paths, test several tabs and repeated starts for one Codex process. One external status should be applied once in every existing sidebar with no plugin relay, repeated starts should leave one watcher, and closing the lowest-ID sidebar should deterministically transfer leadership. July 2026 macOS reference measurements observed one roughly 25 MiB watcher after two starts, zero after process exit, and 20 ordinary hook invocations in about 1.18 seconds (59 ms each). These values characterize the current Python cost; they are not portable regression thresholds. Replacing the bridge with a smaller native helper remains out of scope (`DEVELOPMENT.md`, `openspec/changes/archive/2026-07-20-optimize-status-performance/`).

Deploy the hooks and WASM together: watcher/journal ownership lives in Python while broadcast handling and cache migration live in the plugin. The completed implementation checklist and archive-before-merge evidence are recorded under `openspec/changes/archive/2026-07-20-optimize-status-performance/`.
...
- payload validation, timestamps, pane reuse, clear tombstones, approval recovery, turn-terminal completion, cleanup, peer discovery, snapshots, server-sharded cache recovery and legacy migration, corrupt/oversized cache filtering, and stale-recovery rejection;
- exact-record focus-edge acknowledgement, cached-focus race prevention, leader election and turnover, cross-instance focus observations, attached-client versus plugin-local tab focus, newer-status invalidation, acknowledgement-before-status ordering, and snapshot recovery;
...
`mise run test` runs `hooks/common`, `hooks/codex`, and `hooks/claude` unittest suites. Common coverage owns normalized construction, validation, journal ordering, snapshot delegation, best-effort publication, colocated installation, and locked cleanup of explicit-clear and six-hour stale different-session pane records. Adapter coverage owns native lifecycle mappings, post-approval recovery, Codex/Zellij ancestry discovery, watcher metadata refresh and lock deduplication, Claude `SessionEnd` cleanup and exact notification JSON, and Codex notifier forwarding. The watcher tests replace the complete `select` dependency so the kqueue and polling branches are deterministic on both macOS and Linux instead of depending on host-only module attributes.

### WASM and specification checks

Always pair host tests with a WASM build because host success does not prove the runtime target or `_start` module shape. `mise run check` is the complete pre-PR gate: formatting, Rust and Python tests, Clippy with warnings denied, debug WASM, strict validation of all OpenSpec artifacts, shell syntax checks for documentation update, hook packaging, and release scripts, and `git diff --check`. `mise run release` depends on that gate before building release WASM. The archived pane-aware completion record (`openspec/changes/archive/2026-07-17-nest-panes-under-tabs/tasks.md`) provides additional live/headless verification evidence.

The merged behavior contract is `openspec/specs/`, and completed proposals and rationale are under `openspec/changes/archive/`. After a user-visible change is accepted, run `openspec archive <change-name> -y` on its feature branch; this syncs the delta into the baseline and moves the change artifacts into the archive. A completed change is not merge-ready while it remains active under `openspec/changes/`. Rerun `mise run check` after archiving, then commit the baseline-spec update, archived artifacts, and generated documentation on the same branch. The accepted flexible-layout and native-boundary-drag change, including its disposable-session verification and completed local gate, is archived at `openspec/changes/archive/2026-07-18-add-mouse-resizable-sidebar/`.
...
- `mise run publish -- vX.Y.Z` publishes the checked release WASM, a version-matched `agent-hooks.tar.gz`, and a SHA-256 checksum for each as GitHub Release assets. Before building, `scripts/publish-release` requires authenticated `gh`, a completely clean worktree, the remote default branch, a tag matching `Cargo.toml`, local `HEAD` equal to the fetched remote head, and no existing tag or release. It delegates archive construction to `scripts/package-agent-hooks`, whose output contains the common runtime, Codex adapter/notifier/template, and Claude Code adapter/template from the same commit as the WASM. `gh release create` stages uploads as a draft and publishes only after all four assets succeed; after an interrupted attempt, inspect drafts with `gh release list` before retrying.
- Published assets are installable without Rust or mise from the stable `/releases/latest/download/zellij_vertical_tab.wasm` and `/releases/latest/download/agent-hooks.tar.gz` URLs and should be verified against each adjacent checksum with `shasum -a 256 -c`. Release notes name the matching `zellij-tile`/Zellij version because one release supports one plugin ABI. Existing Codex and Claude configuration must be merged with the bundled templates rather than overwritten; see `README.md` for the checksum-verified commands.
- If an existing release predates hook distribution, check out its exact tag, run `scripts/package-agent-hooks <staging-dir>`, upload `agent-hooks.tar.gz` and `agent-hooks.tar.gz.sha256` with `gh release upload <tag>`, and verify the public latest-download URLs. Never backfill a release with hooks from a different source version. This backfill and checksum verification are complete for `v0.1.0` (`openspec/changes/archive/2026-07-26-publish-github-release-assets/tasks.md`).
