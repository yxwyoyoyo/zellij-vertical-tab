...
Users who do not need a source checkout can instead download `zellij_vertical_tab.wasm` and its SHA-256 checksum from the latest GitHub Release; the release notes identify the required matching Zellij ABI. The release workflow also packages the common bridge plus Codex and Claude Code adapters and templates as a checksum-verified, version-matched `agent-hooks.tar.gz`, so agent integration does not require cloning the repository. Maintainers publish these assets explicitly from a clean, pushed default branch through the guarded [`mise run publish -- vX.Y.Z` workflow](development.md#release-and-maintenance-notes), not through hosted CI (`README.md`, `scripts/publish-release`, `scripts/package-agent-hooks`).
