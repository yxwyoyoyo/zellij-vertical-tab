...
description: Runtime architecture of the pane-aware Zellij sidebar, covering native nested-list rendering, pane-owned Codex status, leader-based peer synchronization, bounded same-server recovery, input, and Zellij-owned mouse-resizable layout constraints.
...
Codex hooks ──host journal + Zellij broadcast pipe──> pane-keyed AgentRecord in every sidebar
...
The Python bridges use `status_store.py` to write each validated lifecycle event before pipe publication. Records are isolated by Zellij server PID and terminal pane, serialized under an advisory per-pane lock, ordered by timestamp, session, turn, and terminal-completion rules, and replaced atomically. During `SessionStart`, one bounded ancestor traversal discovers both the nearest Codex PID and Zellij server PID. Repeated starts atomically refresh metadata but a nonblocking advisory lock allows at most one long-lived watcher for that server/Codex PID pair. The watcher uses macOS `kqueue` process-exit notification when available, otherwise polls every two seconds, then journals and publishes `clear` using the latest metadata. Session start also prunes only numeric host-journal shards whose PID is demonstrably absent; permission errors are treated as alive. This host journal remains available when a detached session has no plugin runtime to receive an undirected pipe (`hooks/codex/agent_status.py`, `hooks/codex/status_store.py`).
...
- **Codex hooks:** `hooks/codex/agent_status.py` maps lifecycle hooks, coordinates one locked exit watcher per Codex PID, and serves recovery snapshots; `agent_notify.py` covers `agent-turn-complete` paths and can forward an existing notifier; both journal valid events through `status_store.py` before pipe publication.
